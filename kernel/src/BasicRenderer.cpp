...
void BasicRenderer::drawRectangle(int posX, int posY, int Width, int Height, uint32_t Colour){
    for(int i=0; i<Height; i++){
        for(int j=0; j<Width; j++){
            PutPix(posX+j, posY+i, Colour);
        }
    }
}

void BasicRenderer::drawHorizontalLine(int posX, int posY, int Width, uint32_t Colour){
    for(int i=0; i<Width; i++){
        PutPix(posX+i, posY, Colour);
    }
}

void BasicRenderer::drawVerticalLine(int posX, int posY, int Height, uint32_t Colour){
    for(int i=0; i<Height; i++){
        PutPix(posX, posY+i, Colour);
    }
}

void BasicRenderer::drawRectangleNoFill(int posX, int posY, int Width, int Height, uint32_t Colour){
    drawHorizontalLine(posX, posY, Width, Colour);
    drawHorizontalLine(posX, posY+Height, Width, Colour);
    drawVerticalLine(posX, posY, Height, Colour);
    drawVerticalLine(posX+Width, posY, Height, Colour);
}
